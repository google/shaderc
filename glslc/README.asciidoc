...
`-M` generates *make* dependencies, outputs a rule suitable for *make*
describing the dependencies of the main source file. Instead of outputting
the result of preprocessing, the preprocessor outputs one *make* rule
containing the SPIR-V object file name for that source file, a colon, and the
names of all the included files.
...
filename formed by append *.d* to the name of the file containing compilation
results.
...
|glslc -S -MD main.vert |main.vert.s  |main.vert.s.d  |main.vert.s: main.vert
